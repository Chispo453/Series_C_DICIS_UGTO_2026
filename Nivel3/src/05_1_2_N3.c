 ...
 * ── NIVEL 3 ──
 * @file 5_1_2_N3.c
 * @brief Aproximacion de 1/2 con serie telescopica por recurrencia O(n).
 *
 * @details
 ...
 *   ter_1     = 1/(1*3)
 *   ter_{i+1} = ter_i * (2i-1)*(2i+1) / [(2i+1)*(2i+3)]
 *   = ter_i * (2i-1) / (2i+3)
 *
 * Derivacion:
 *   ter_i     = 1/[(2i-1)(2i+1)]
 *   ter_{i+1} = 1/[(2i+1)(2i+3)]
 *   ter_i * (2i-1)/(2i+3)
 *
 * @par Entrada
 * @param n  Entero >= 1.
 ...
 *
 * @warning
 * Serie telescopica: la suma parcial tiene forma cerrada
 * S_n = n/(2n+1), lo que confirma la convergencia a 1/2.
 */
#include <stdio.h>
#include <math.h>
